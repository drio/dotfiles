...
function screenshot_timestamp()
	...
	t = { args = { "exiftool", "-T", "-DateTimeOriginal", path }, capture_stdout = true, capture_stderr = true }
	...
	if res and res.error == nil then
		...
		local xyear, xmonth, xday, xhour, xminute, xseconds, xmillies, xoffset = res.stdout:match(pattern)
		local convertedTimestamp =
			os.time({ year = xyear, month = xmonth, day = xday, hour = xhour, min = xminute, sec = xseconds })
		...
		dt = dir
			.. fname
			.. os.date("-%Y-%m-%d-%H-%m-%S", convertedTimestamp + pos)
			.. string.sub(pos % 1, 2)
			.. "."
			.. fmat
		-- save screenshot
		mp.commandv("screenshot-to-file", dt, "subtitles")
		print("screenshot to: " .. dt)
	else
		mp.msg.warn("FAIL!")
		mp.msg.warn(res)
	end
...
mp.add_key_binding("s", "screenshot-timestamp", screenshot_timestamp)
